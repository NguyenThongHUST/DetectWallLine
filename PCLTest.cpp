...
// 
const double stepSize = 0.01; // 0.01
...
cv::Mat rotate2DImage(cv::Mat& input_img) {
    ...
    cv::warpAffine(input_img, rotated_image, rotation_matrix, cv::Size(width, height));
 
    cv::imwrite(outPath + "img/rotate_image.jpg", rotated_image);
    return rotated_image;
}

PointCloudPtr radiusOutlierRemoval(PointCloudPtr& cloud){
...
// filtering data pre processing
PointCloudPtr statisticalRemovalOutlier(PointCloudPtr cloud) {
    PointCloudPtr cloud_filtered{new pcl::PointCloud<pcl::PointXYZ>};
...
cv::Mat createDepthImg(PointCloudPtr cloud)
{
    ...
    Canny(img, gray, 50, 200, 5);
    cv::imwrite(outPath + "img/canny.jpg", gray);
...
cv::Mat houghLineDetection(cv::Mat img)
{
    ...
    {
        cv::Vec4i l = lines[i];
        line(cdst, cv::Point(l[0], l[1]), cv::Point(l[2], l[3]), cv::Scalar(0, 0, 255), 2, cv::LINE_AA);
    }
...
cv::Mat houghLineDetectionTest(cv::Mat img) 
{
    cv::Mat dst;
    cv::Mat img_blur;
    cv::Mat sobelxy;

    cv::GaussianBlur(img, img_blur, cv::Size(5, 5), 3);
    cv::imwrite(outPath + "img/blur.jpg", img_blur);


    cv::Sobel(img_blur, sobelxy, CV_64F, 1, 1, 5);
    cv::imwrite(outPath + "img/sobelxy.jpg", sobelxy);

    cv::Canny(img_blur, dst, 100, 200, 5, false);
    ...
    {
        cv::Vec4i l = lines[i];
        line(dst, cv::Point(l[0], l[1]), cv::Point(l[2], l[3]), cv::Scalar(0, 0, 255), 2, cv::LINE_AA);
    }
    cv::imwrite(outPath + "img/hld.jpg", dst);
...
int main(int argc, char** argv)
{
    ...
    std::string file_out_path = outPath + "Cropted_Point_index.pcd";
    std::ifstream inputFile("InputData/Cropted_Point_index.txt");
    ...
    std::cout << "Min data: " << std::fixed << std::setprecision(5) 
        << pointMin.x << " " << pointMin.y << " " << pointMin.z << std::endl;


    for (int i = 0; i < inputCloud->size(); i++)
    {
        inputCloud->points[i].x -= pointMin.x;
        inputCloud->points[i].y -= pointMin.y;
        inputCloud->points[i].z -= pointMin.z;
    }

    pcl::io::savePCDFile(file_out_path, *inputCloud, false);

    // end

    
    cloud = statisticalRemovalOutlier(inputCloud);
    cloud = createWallPCD(cloud);
    cloud = removeOutliersFromWallPCD(cloud, 1);
    depthImg = createDepthImg(cloud);
    
    depthImg = cv::imread(outPath + "img/depth.jpg");
    rotateImg = rotate2DImage(depthImg);
    lineDetection(rotateImg);
